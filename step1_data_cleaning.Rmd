...
# Quick visual check 
  ...
  plot(all_traits$RR, 1/sqrt(all_traits$VRR) ) # NOTE: Use precision instead, which is 1/se, so need to sqrt. There are some very high precision values here, did you just check these data are correct?
  hist(all_traits$CVR, breaks = 50)
  plot(all_traits$CVR, 1/sqrt(all_traits$VCVR))

# Visualizing mean-variance relationship
 # Correlation of control group
   plot(log(all_traits$con_mean), log(all_traits$con_sd))
   cor.test(all_traits$con_mean, all_traits$con_sd) # 0.951138 
 # Correlation of experiment group
   plot(log(all_traits$exp_mean), log(all_traits$exp_sd))
   cor.test(all_traits$exp_mean, all_traits$exp_sd) # 9964369 
```

### 5. Splitting dataset
Preparing specific trait categories
...
##################### Note all above code from "Define Trait Categories" can be condensed as follows:
           type_behav <- c("learning(MWM)", "memory(MWM)", "memory(NOR)", "anxiety(EPM)", "anxiety(OFT)")
           type_genes <- c("BDNF", "TrkB", "VEGF", "reelin", "GFAP", "syptophysin")
          type_neuron <- c("neuron count", "brain size")
           type_other <- c("mitochondrial biogenesis", "DNA methylation")
    
  all_traits <- all_traits %>%
            mutate(grouped_traits = ifelse(response_type %in% type_behav, "behavior", 
                                           ifelse(response_type %in% type_genes, "genes",
                                                 ifelse(response_type %in% type_neuron, "brain development", 
                                                       ifelse(response_type %in% type_other, "others", response_type)))))

######################          


 # Split into specific trait categories;  # also can use filter() to split: all_traits %>% filter(response_type == 'trait')
             neuron_count <- all_traits[all_traits$response_type == 'neuron count',]
 ...
 # we are also interested in the component of behavior, so we divided behavior into cognition and anxiety
  # cognition <- rbind(learning_MWM, memory_MWM, memory_NOR)
  # anxiety <- rbind(anxiety_OFT, anxiety_EPM)



# Pre-processed subsets for study-level correlations among traits; NOTE: NOte entirely clear what is happening here and why this is done. Can you annote some more to explain. Is this for measuring correlations between effects within studies?
 ...
 # The minimum effect size within each study
...
NOTE: Above you have a lot of repeated code. I don't want to break anything, but whenever you have this try to wrap it into a function. For example, You can do all the steps above and the saving below (step 7) as follows:

# Just an example here with two of the datasets you used
data_list <- list(neuron_count=neuron_count, 
                    brain_size=brain_size)
names <- names(data_names)

# Here is the function that does it all. You just apply once in mapply function. This can even go directly to your function.R script so you don't need it here. Note that you can change the path to whatever directory you want. It will test and print whether a matrix is positive definite and then save it all after building matrix. This can be improved quite a bit yet still, but this is a first pass that can really reduce the amount of code you have 
  create_vcv_matrix <- function(data, name, path = "."){
    # Using data, create matrices for CVR and RR. Note that much of the code stays the same except the Rr and CVR and data frame
     tmp_matrix_RR <- make_VCV_matrix(data = data,  V = "VRR",  cluster = 'cohort_ID', type = 'vcv', obs = 'ES_ID', rho = 0.5)
    tmp_matrix_CVR <- make_VCV_matrix(data = data, V = "VCVR", cluster = 'cohort_ID', type = 'vcv', obs = 'ES_ID', rho = 0.5)

    # Check that matrices are positive definite
    print(is.positive.definite(tmp_matrix_RR))
    print(is.positive.definite(tmp_matrix_CVR))

    # Save matrix
     namesRR <- paste0(path, "/cov_VRR_", name)
    namesCVR <- paste0(path,"/cov_VCVR_", name)

     saveRDS(tmp_matrix_RR, file = namesRR)
     saveRDS(tmp_matrix_CVR, file = namesCVR)  
  }

# Ok, now use the function once and apply to the list of dataframes
mapply(function(x,y) create_vcv_matrix(x,y, path = "."), x = data_list, y = names)
...
# Variance-covariance matrices NOTE: Again everything from 410-448 can go and be replaced with the function above
 # RR
